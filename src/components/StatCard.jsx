    ...
    return (
        <>
            ...
            <div style={{ background: CARD_BG, borderRadius: 14, padding: 20, boxShadow: "0 2px 16px rgba(0,0,0,0.2)", border: `1px solid ${CARD_BORDER}`, marginBottom: 24 }}>
                <ResponsiveContainer width="100%" height={400}>
                    <ScatterChart margin={{ top: 10, right: 20, bottom: 30, left: 10 }}>
                        ...
                        <XAxis type="number" dataKey="xg" name="xG" domain={[0.8, 2.2]} tick={{ fill: GRAY, fontSize: 11 }} label={{ value: "Expected Goals (xG) per match", position: "bottom", fill: GRAY, fontSize: 12, offset: 15 }} />
                        <YAxis type="number" dataKey="goals" name="Goals" domain={[0.5, 3.5]} tick={{ fill: GRAY, fontSize: 11 }} label={{ value: "Actual Goals per match", angle: -90, position: "insideLeft", fill: GRAY, fontSize: 12 }} />
                ...
                <div style={{ display: "flex", gap: 20, justifyContent: "center", marginTop: 8, fontSize: 11, color: GRAY }}>
            ...
            <div style={{ background: CARD_BG, borderRadius: 14, padding: 20, boxShadow: "0 2px 16px rgba(0,0,0,0.2)", border: `1px solid ${CARD_BORDER}` }}>
                {TEAMS.slice(0, 15).sort((a, b) => b.xgOver - a.xgOver).map((t, i) => (
                    <div key={t.name} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: i < 14 ? `1px solid ${NAVY_LIGHT}` : "none" }}>
                        <span style={{ fontSize: 22 }}>{t.flag}</span>
                        <span style={{ fontSize: 13, fontWeight: 700, flex: 1, color: WHITE }}>{t.name}</span>
                        <div style={{ width: 160, height: 8, background: NAVY_LIGHT, borderRadius: 4, position: "relative", flexShrink: 0 }}>
